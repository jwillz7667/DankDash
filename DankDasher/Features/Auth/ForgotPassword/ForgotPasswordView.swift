...
/// Password-reset entry point. Reused from the consumer target — the
/// underlying ``ForgotPasswordFeature`` calls the same backend reset
/// endpoint and the UX intentionally never confirms or denies that an
    ...
    ScrollView {
      VStack(spacing: DankSpacing.lg) {
        VStack(spacing: DankSpacing.sm) {
          ...
          Text("Enter the email tied to your driver account.")
            .font(DankFont.body)
