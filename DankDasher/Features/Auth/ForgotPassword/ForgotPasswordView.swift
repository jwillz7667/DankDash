...
/// Password-reset flow, reused from the consumer target — the underlying
/// ``ForgotPasswordFeature`` calls the same backend reset endpoints. Two
/// steps: request a code by email, then redeem it with a new password. The
/// request step intentionally never confirms or denies that an email is
...
struct ForgotPasswordView: View {
  ...
  private var headerSubtitle: String {
    switch store.step {
    case .request: "Enter the email tied to your driver account."
