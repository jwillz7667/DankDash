...
/// Launch-time session gate: a stored driver session exists and must be
/// unlocked with Face ID / device authentication before `loadingDriver`
/// runs. The reducer auto-attempts on appear, so the happy path is one
/// Face ID glance — the buttons only matter after a canceled or failed
/// prompt.
  ...
  var body: some View {
    VStack(spacing: DankSpacing.lg) {
      ...
      VStack(spacing: DankSpacing.sm) {
        ...
        Text("Unlock to get back on the road.")
          .font(DankFont.body)
