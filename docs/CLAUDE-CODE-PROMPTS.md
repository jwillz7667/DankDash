...
We are now starting Phase <N> — <Phase Name>.

Read `docs/CLAUDE-CODE-PHASES.md` section "PHASE <N>" carefully. Also re-read `PROGRESS.md` to see what was accomplished in prior phases. Pay particular attention to any conventions established earlier — your code must match the patterns already in the codebase, not invent new ones.
