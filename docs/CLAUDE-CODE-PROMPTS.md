...
We are now starting phase 9 Realtime Service (Socket.io).

Read `docs/CLAUDE-CODE-PHASES.md` section "PHASE 9" carefully. Also re-read `PROGRESS.md` to see what was accomplished in prior phases. Pay particular attention to any conventions established earlier — your code must match the patterns already in the codebase, not invent new ones.
